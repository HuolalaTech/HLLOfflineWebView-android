...
public class DefaultRequest implements IOfflineRequest {

    private String TAG = DefaultRequest.class.getSimpleName();
    ...
    public void requestPackageInfo(String bizName, String version, RequestCallback<OfflinePackageInfo> callback) {
        ...
        HttpUrl httpUrl = HttpUrl.parse(Constants.BASE_URL);
        if (httpUrl == null) {
            if (OfflineWebManager.getInstance().isDebug()) {
                throw new IllegalStateException("parse error url :" + Constants.BASE_URL);
            } else {
                OfflineWebLog.e(TAG, new IllegalStateException("parse error url :" + Constants.BASE_URL));
                return;
            }
        }
        if (callback == null) {
